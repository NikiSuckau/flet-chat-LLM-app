...
class FletChatApp:
    ...
    def build(self, page: ft.Page):
        ...
        def save_settings_click(e):
            ...
            self.backend.summary_system_prompt = settings_view.get_summary_system_prompt()
            ...
            self.settings.summary_system_prompt = settings_view.get_summary_system_prompt()
            self.settings.summary_prompt = settings_view.get_summary_prompt()
            self.settings.summary_temperature = settings_view.get_summary_temperature()
        ...
        def get_previous_entry_text() -> str | None:
            ...
            for i, entry in enumerate(entries):
                if entry.id == diary_view.current_entry_id:
                    return entries[i - 1].text if i > 0 else None
            return entries[-1].text

        diary_view = DiaryView(
            self.backend.stream_diary_question,
            self.backend.stream_diary_summary,
            get_previous_entry_text,
            save_diary_click,
        )
        ...
        def show_settings():
            ...
            settings_view.set_summary_system_prompt(self.backend.summary_system_prompt)
            settings_view.set_summary_prompt(self.backend.summary_prompt)
            settings_view.set_summary_temperature(self.backend.summary_temperature)
