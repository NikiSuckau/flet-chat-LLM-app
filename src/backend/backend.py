...
class ChatBackend:
    ...
    DEFAULT_SUMMARY_PROMPT = (
        "Fasse den folgenden Tagebucheintrag in einem Satz zusammen:\n"
    )
    ...
    def stream_diary_summary(
        ...
        """Yield a summary of a diary entry token by token."""

        prompt_text = (prompt or self.summary_prompt) + f"{diary_text}"
        max_tokens = max_tokens if max_tokens is not None else self.summary_max_tokens
        temperature = temperature if temperature is not None else self.summary_temperature
